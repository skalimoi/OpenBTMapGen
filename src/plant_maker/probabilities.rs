...
use crate::plant_maker::config::{GreyscaleImage, Vegetation};
...
pub fn calculate_probabilities(
    ...
    println!(
        "Main reason for not growing (except plant_maker demand): {}",
        ["insolation", "plant_maker depth", "water demand", "only plant_maker demand"][location_factor_with_max_reasons_for_not_growing.0]
    );
    probabilities
}
